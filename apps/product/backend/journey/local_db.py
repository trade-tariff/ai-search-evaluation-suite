...
and duty calculation (real measures, components, geographical memberships).
...
# Terminology: a row of kg.commodity_facets is a FACT (one code's value on a
# facet axis, e.g. material_upper: rubber); the FACET is the facet_key itself.
# The sources below are label/search-reference vocabulary MIRRORED into the
# facts table for provenance - excluded from the facts retrieval legs so that
# vocabulary only counts once, via the label/curated legs.
...
def _kg_fact_source_filter(alias: str, excluded_sources: Optional[list[str]] = None) -> tuple[str, list[str]]:
    """Keep label/search vocabulary in label/composite legs, not KG fact legs."""
    excluded = sorted(KG_FACT_ALIAS_SOURCES | set(excluded_sources or []))
...
def _query_tokens(query: str) -> list[str]:
    """Trader-query tokens for tolerant label/fact matching."""
    stop = {
        "and", "for", "the", "with", "from", "into", "onto", "that", "this",
        "their", "there", "are", "not", "other", "goods", "product",
    }
    tokens = []
    for token in re.findall(r"[A-Za-z0-9][A-Za-z0-9+-]{2,}", query.lower()):
        norm = token.strip("-+")
        if len(norm) >= 3 and norm not in stop and norm not in tokens:
            tokens.append(norm)
    return tokens[:10]


def _labels_leg(query: str, limit: int) -> list[dict]:
    """Retrieve against production AI labels: descriptions, synonyms and colloquial terms.

    These labels are lexical candidate targets, not commodity facts. Unlike `plainto_tsquery`, this
    leg tolerates partial matches so a trader phrase like "pre workout drink
    powder" can still hit labels containing "drink mix" and "powdered mix".
    """
    ...
    with _conn() as c, c.cursor() as cur:
        cur.execute(
            f"""
            WITH token_input AS (
              SELECT %s::text[] AS tokens
            ),
            label_rows AS (
              SELECT
                gl.goods_nomenclature_item_id AS commodity_code,
                gl.description AS description,
                lower(
                  COALESCE(gl.description, '') || ' ' ||
                  COALESCE(gl.original_description, '') || ' ' ||
                  COALESCE(array_to_string(gl.synonyms, ' '), '') || ' ' ||
                  COALESCE(array_to_string(gl.colloquial_terms, ' '), '') || ' ' ||
                  COALESCE(array_to_string(gl.known_brands, ' '), '') || ' ' ||
                  COALESCE(gl.labels::text, '')
                ) AS label_text
              FROM {SCHEMA}.goods_nomenclature_labels gl
              WHERE COALESCE(gl.expired, false) = false
            ),
            scored AS (
              SELECT
                lr.commodity_code,
                ...
                (
                  SELECT count(*)
                  FROM token_input ti, unnest(ti.tokens) AS t(token)
                  WHERE lr.label_text ILIKE ('%%' || t.token || '%%')
                ) AS token_hits,
                cardinality((SELECT tokens FROM token_input)) AS token_count,
                word_similarity(%s, lr.label_text) AS word_score,
                similarity(COALESCE(lr.description, ''), %s) AS description_score
              FROM label_rows lr
              WHERE EXISTS (
                SELECT 1
                FROM token_input ti, unnest(ti.tokens) AS t(token)
            ...
            SELECT commodity_code, description,
                   (
                     token_hits::float / GREATEST(token_count, 1)
                     + word_score
                     + description_score
                   ) AS score,
                   token_hits
            FROM scored
            ORDER BY score DESC, token_hits DESC
            LIMIT %s
            """,
            (tokens, query, query, limit),
        )
        return [
            {
                "commodity_code": r["commodity_code"],
...
def _facts_leg(query: str, limit: int, exclude_sources: Optional[list[str]] = None) -> list[dict]:
    ...
    source_filter, source_excl = _kg_fact_source_filter("cf", excl)
    with _conn() as c, c.cursor() as cur:
        cur.execute(
            f"""
            WITH q AS (SELECT plainto_tsquery('english', %s) AS tsq)
...
_LEAF_CACHE: dict = {"set": None}


def _declarable_leaf_set() -> frozenset:
    """Cached canonical declarable leaf set: suffix-80 indent-leaves minus chapter pseudo-codes."""
    if _LEAF_CACHE["set"] is None:
        try:
            with _conn() as c, c.cursor() as cur:
                cur.execute(f"""
                  WITH live AS (
                    SELECT DISTINCT ON (gn.goods_nomenclature_sid)
                           gn.goods_nomenclature_item_id AS code, gn.producline_suffix AS suffix,
                           gni.number_indents AS indent
                    FROM {SCHEMA}.goods_nomenclatures gn
                    JOIN {SCHEMA}.goods_nomenclature_indents gni
                      ON gni.goods_nomenclature_sid = gn.goods_nomenclature_sid
                    WHERE gn.validity_end_date IS NULL AND gni.validity_start_date <= now()
                    ORDER BY gn.goods_nomenclature_sid, gni.validity_start_date DESC
                  ),
                  ordered AS (
                    SELECT code, suffix, indent,
                           LEAD(indent) OVER (PARTITION BY left(code, 2) ORDER BY code, suffix) AS nxt
                    FROM live
                  )
                  SELECT code FROM ordered WHERE suffix = '80' AND (nxt IS NULL OR nxt <= indent)
                """)
                _LEAF_CACHE["set"] = frozenset(
                    f for f in (_flat(r["code"]) for r in cur.fetchall())
                    if not re.match(r"^\d{2}0{8}$", f))
        except Exception as e:
            print(f"[declarable set] {type(e).__name__}: {e}")
            _LEAF_CACHE["set"] = frozenset()
    return _LEAF_CACHE["set"]
...
def retrieve_candidates(
    ...
    if use_labels:
        try:
            legs.append(_labels_leg(query, limit))
            caps.append(1.0)
        except Exception as e:
            print(f"[retrieval labels] {e}")
    try:
    ...
    pool = _rrf_fuse(legs, limit=max(limit * 4, 400) if filter_declarable else limit, k=rrf_k, leg_caps=caps)
    if filter_declarable:
        leaves = _declarable_leaf_set()
        if leaves:
            kept = [c for c in pool if _flat(c["commodity_code"]) in leaves]
            if kept:
                pool = kept
    return pool[:limit]
...
def _parent_code_chain(code: str) -> list[str]:
    ...
    subheading-padded form), heading (7307000000) and chapter (7300000000).
    Trailing zeros mark the higher level. We enumerate the prefix-zero-padded
    forms at 8, 6, 4, 2 and let the INTERSECT with actually-existing codes
    happen in SQL. The chapter level matters: e.g. EU TCA preference (type
    142, geo 1013) measures attach at chapter-padded codes like 2200000000.
...
def applicable_measures(
    ...
    children unless explicitly overridden. Without this, duty calc and cert
    ...
    with _conn() as c, c.cursor() as cur:
        ...
        out = []
        # Track (measure_type_id, geo, additional code) seen at narrower scope
        # so a broader measure attached at heading level doesn't shadow a
        # more-specific override on the exact code. The additional code is part
        # of the key so company-specific anti-dumping variants and VATZ
        # zero-rate rows survive alongside their siblings.
        seen: set[tuple] = set()
        for r in cur.fetchall():
            attached_code = r["goods_nomenclature_item_id"]
            key = (
                r["measure_type_id"],
                r["geographical_area_id"],
                r["additional_code_type_id"],
                r["additional_code_id"],
            )
            inherited = attached_code != flat
            # Skip if a more-specific measure for the same (type, geo, addl
            # code) already claimed precedence (we order by specificity DESC so
            # more-specific comes first).
            if key in seen and inherited:
                continue
            seen.add(key)
            additional_code = None
            if r["additional_code_type_id"] or r["additional_code_id"]:
                additional_code = f"{r['additional_code_type_id'] or ''}{r['additional_code_id'] or ''}"
            out.append({
...
def import_requirements(
    ...
    Per codex's review: declaration & duty should both consume one resolver
    ...
    Returns:
        {
            ...
            "duty_measures": [...],         # type 103/142 only (duty-bearing)
    ...
    duty_measures = [m for m in measures if m["measure_type_id"] in {"103", "142", "122"}]
...
def _cert_documents_for_chain(flat_code: str, as_of: str | None = None) -> list[dict]:
    ...
    full parent chain. This is the deterministic version that declaration.py
...
def commodity_vat_rate(code: str, as_of: str | None = None) -> Optional[float]:
    """Default VAT rate (%) for a commodity from its type-305 VAT measures.

    UK VAT measures attach along the parent chain at geo 1400, often with a
    VATZ additional-code variant for the zero rate. The most specific
    attachment wins; when several rates coexist there (standard + zero
    variants), return the highest - it is a wizard default the trader can
    override, so prefer not to under-state. None when no 305 measure exists.
    """
    flat = _flat(code)
    as_of = _as_of_date(as_of)
    chain = _parent_code_chain(flat)
    with _conn() as c, c.cursor() as cur:
        cur.execute(
            f"""
            SELECT m.goods_nomenclature_item_id AS attached_at, mc.duty_amount,
                   COALESCE(m.additional_code_id, '') AS additional_code
            FROM {SCHEMA}.measures m
            JOIN {SCHEMA}.measure_components mc ON mc.measure_sid = m.measure_sid
            WHERE m.goods_nomenclature_item_id = ANY(%s)
              AND m.measure_type_id = ANY(%s)
              AND (m.validity_end_date IS NULL OR m.validity_end_date > %s::date)
              AND m.validity_start_date <= %s::date
              AND mc.duty_amount IS NOT NULL
            ORDER BY LENGTH(REPLACE(m.goods_nomenclature_item_id, '0', '')) DESC
            """,
            (chain, list(VAT_TYPES), as_of, as_of),
        )
        rows = cur.fetchall()
    if not rows:
        return None
    # Additional-code variants (VATZ/ATZ zero-rate reliefs) only apply when the
    # declaration carries that code, so the wizard default must come from the
    # plain measures; coded variants are a fallback when nothing else exists.
    plain = [r for r in rows if not r["additional_code"]]
    pool = plain or rows
    most_specific = pool[0]["attached_at"]
    rates = {float(r["duty_amount"]) for r in pool if r["attached_at"] == most_specific}
    return max(rates) if rates else None


def excise_rate_components(code: str, as_of: str | None = None) -> list[dict]:
    """Live type-306 excise duty components for the most specific attachment on
    the code's parent chain.

    Returns one row per component of the chosen measure:
    {duty_expression_id, duty_amount, monetary_unit_code, measurement_unit_code,
     additional_code, attached_at}. Duty expression 15 is a minimum (the
    cigarette Minimum Excise Tax floor). Empty list when the DB is unavailable
    or no excise measure applies, so callers can fall back to static rates.
    """
    try:
        ...
        with _conn() as c, c.cursor() as cur:
            cur.execute(
                ...
                SELECT m.measure_sid,
                       m.goods_nomenclature_item_id AS attached_at,
                       COALESCE(m.additional_code_id, '') AS additional_code,
                       mc.duty_expression_id, mc.duty_amount,
                       mc.monetary_unit_code, mc.measurement_unit_code
                FROM {SCHEMA}.measures m
                JOIN {SCHEMA}.measure_components mc ON mc.measure_sid = m.measure_sid
                WHERE m.goods_nomenclature_item_id = ANY(%s)
                  AND m.measure_type_id = '306'
                  AND (m.validity_end_date IS NULL OR m.validity_end_date > %s::date)
                  AND m.validity_start_date <= %s::date
                  AND mc.duty_amount IS NOT NULL
                ORDER BY LENGTH(REPLACE(m.goods_nomenclature_item_id, '0', '')) DESC,
                         COALESCE(m.additional_code_id, ''), mc.duty_expression_id
                """,
                (chain, as_of_date, as_of_date),
            )
            rows = [dict(r) for r in cur.fetchall()]
        if not rows:
            return []
        attached = rows[0]["attached_at"]
        at_level = [r for r in rows if r["attached_at"] == attached]
        plain_sids = sorted({r["measure_sid"] for r in at_level if not r["additional_code"]})
        all_sids = sorted({r["measure_sid"] for r in at_level})
        chosen_sid = (plain_sids or all_sids)[0]
        return [r for r in at_level if r["measure_sid"] == chosen_sid]
    except Exception as e:
        print(f"[excise components] {type(e).__name__}: {e}")
        return []


# --- Declarable-leaf checks (TARIC indent tree) --------------------------

def _chapter_tree_rows(chapter: str) -> list[dict]:
    """Live rows for one chapter in (item_id, suffix) order with their current
    indent. The TARIC tree is implicit in that ordering: a row's descendants
    are the rows that follow it with a strictly deeper indent. Indent rows can
    have several live periods, so take the latest started one per sid.
    """
    with _conn() as c, c.cursor() as cur:
        cur.execute(
            f"""
            SELECT DISTINCT ON (gn.goods_nomenclature_sid)
                   ...
                   gn.goods_nomenclature_item_id AS item_id,
            ...
            JOIN {SCHEMA}.goods_nomenclature_indents gni
              ON gni.goods_nomenclature_sid = gn.goods_nomenclature_sid
            WHERE gn.validity_end_date IS NULL
              AND gni.validity_start_date <= now()
        ...
        rows = [dict(r) for r in cur.fetchall()]
    rows.sort(key=lambda r: (r["item_id"], r["suffix"]))
    return rows


def _descriptions_for_sids(sids: list[int]) -> dict[int, str]:
    if not sids:
        return {}
    with _conn() as c, c.cursor() as cur:
        cur.execute(
            ...
            SELECT DISTINCT ON (goods_nomenclature_sid)
                   goods_nomenclature_sid AS sid, description
            FROM {SCHEMA}.goods_nomenclature_descriptions
            WHERE goods_nomenclature_sid = ANY(%s)
            ORDER BY goods_nomenclature_sid, goods_nomenclature_description_period_sid DESC
            """,
            (sids,),
        )
        return {r["sid"]: (r["description"] or "").strip() for r in cur.fetchall()}


def is_declarable_leaf(code: str) -> bool:
    """True when `code` is a live suffix-80 product line with no deeper-indent
    child, i.e. the level a CDS declaration actually accepts.

    Safe fallback: True on any DB error so the duty flow is never blocked by
    infrastructure trouble.
    """
    flat = _flat(code)
    if re.match(r"^\d{2}0{8}$", flat):   # chapter pseudo-code XX00000000 is never declarable
        return False
    try:
        rows = _chapter_tree_rows(flat[:2])
        for i, r in enumerate(rows):
            if r["item_id"] == flat and r["suffix"] == "80":
                nxt = rows[i + 1] if i + 1 < len(rows) else None
                return nxt is None or int(nxt["indent"] or 0) <= int(r["indent"] or 0)
        return False
    except Exception as e:
        print(f"[leaf check] {type(e).__name__}: {e}")
        return True


def declarable_leaf_children(code: str, limit: int = 30) -> list[dict]:
    """Declarable (suffix-80, leaf-by-indent) descendants of `code`, for the
    "this code needs one more level of detail" drill-down.

    Safe fallback: [] on any DB error.
    """
    flat = _flat(code)
    try:
        rows = _chapter_tree_rows(flat[:2])
        start = next(
            (i for i, r in enumerate(rows) if r["item_id"] == flat and r["suffix"] == "80"),
            None,
        )
        if start is None:
            return []
        base_indent = int(rows[start]["indent"] or 0)
        leaves: list[dict] = []
        for i in range(start + 1, len(rows)):
            r = rows[i]
            if int(r["indent"] or 0) <= base_indent:
                break
            if r["suffix"] != "80":
                continue
            nxt = rows[i + 1] if i + 1 < len(rows) else None
            if nxt is None or int(nxt["indent"] or 0) <= int(r["indent"] or 0):
                leaves.append({"sid": r["sid"], "commodity_code": r["item_id"]})
            if len(leaves) >= limit:
                break
        descs = _descriptions_for_sids([leaf["sid"] for leaf in leaves])
        return [
            {"commodity_code": leaf["commodity_code"], "description": descs.get(leaf["sid"], "")}
            for leaf in leaves
        ]
    except Exception as e:
        print(f"[leaf children] {type(e).__name__}: {e}")
        return []


# --- Best applicable customs duty ---------------------------------------
...
# Public name for duty.py's trade-remedy / suspension / quota handling.
component_rate = _component_rate


def best_applicable_duty(code: str, country_code: str, as_of: str | None = None) -> dict:
...
# a full `uk` tariff schema. In that mode, the e2e trader journey runs against
# the bundled thin-slice facet data. Live Postgres remains preferred whenever it
# is reachable, unless JOURNEY_FIXTURE_MODE=1 is set.
...
_DB_PROBE_TTL_SECONDS = 30.0
_db_probe: dict[str, Any] = {"checked_at": None, "ok": False}


def _live_db_available() -> bool:
    """TTL-cached liveness probe (was a boot-time lru_cache) so mid-session DB
    loss or recovery is noticed within ~30s instead of never."""
    if os.environ.get("JOURNEY_FIXTURE_MODE") == "1":
        return False
    now = time.monotonic()
    if _db_probe["checked_at"] is not None and now - _db_probe["checked_at"] < _DB_PROBE_TTL_SECONDS:
        return _db_probe["ok"]
    try:
        with psycopg.connect(DSN, row_factory=dict_row, connect_timeout=2) as c, c.cursor() as cur:
            cur.execute("SELECT 1")
            cur.fetchone()
        ok = True
    except Exception:
        ok = False
    _db_probe["checked_at"] = now
    _db_probe["ok"] = ok
    return ok


def retrieval_health() -> str:
    """Label for ClassifyTurn.retrieval_health.

    'live' when the tariff DB answers, 'fixture' when fixture mode is forced,
    'degraded' when a DB is configured but unreachable (bundled fixtures serve
    the request). 'infra-error' is reserved for the classification layer's own
    failures and is never returned here.
    """
    if os.environ.get("JOURNEY_FIXTURE_MODE") == "1":
        return "fixture"
    return "live" if _live_db_available() else "degraded"
...
_live_best_applicable_duty = best_applicable_duty
_live_import_requirements = import_requirements
_live_cert_documents_for_chain = _cert_documents_for_chain
_live_commodity_vat_rate = commodity_vat_rate
_live_is_declarable_leaf = is_declarable_leaf
_live_declarable_leaf_children = declarable_leaf_children
...
def journey_demo_personas() -> list[dict]:
...
def journey_demo_examples(persona: str | None = None) -> list[dict]:
    """Demo prompts from the live KG/eval corpus.

    This is what the e2e journey should prefer for the local demo. The bundled
    ...
    try:
        with _conn() as c, c.cursor() as cur:
            for code in _DEMO_EXAMPLE_CODES:
                ...
                examples.append(_journey_example_from_eval(row, commodity_row, facet_count))
    except Exception as exc:
        print(f"[journey examples] live KG lookup failed: {exc}")
        return []
    return examples


def _journey_example_from_eval(row: dict, commodity_row: dict, fact_count: int) -> dict:
    ...
    return {
        ...
        # Backwards-compatible alias for existing UI/test consumers. The
        # underlying rows are commodity facts; "facet" is the legacy table name.
        "facet_count": fact_count,
        "seed": seed,
    }
...
def commodity_vat_rate(code: str, as_of: str | None = None) -> Optional[float]:
    if _live_db_available():
        try:
            return _live_commodity_vat_rate(code, as_of=as_of)
        except Exception as e:
            print(f"[vat rate] {type(e).__name__}: {e}")
            return None
    item = _fixture_item(code)
    if item and item.get("vat_rate") is not None:
...
def is_declarable_leaf(code: str) -> bool:
    if _live_db_available():
        return _live_is_declarable_leaf(code)
    return True


def declarable_leaf_children(code: str, limit: int = 30) -> list[dict]:
    if _live_db_available():
        return _live_declarable_leaf_children(code, limit=limit)
    return []


def best_applicable_duty(code: str, country_code: str, as_of: str | None = None) -> dict:
    if _live_db_available():
        return _live_best_applicable_duty(code, country_code, as_of=as_of)
    item = _fixture_item(code)
    if not item:
        return {
            "code": _flat(code),
    ...
    mfn = _fixture_rate(item.get("duty_mfn"), group="MFN", measure_id="fixture-mfn")
    all_measures = [
        {
            "measure_sid": "fixture-mfn",
            "measure_type_id": MFN_TYPE,
            "measure_type_description": "Third country duty (fixture)",
            "geographical_area_id": "1011",
            "geographical_area_description": "MFN",
            "components": [{"duty_amount": item.get("duty_mfn") or 0}],
        }
    ]
    if pref:
        all_measures.append({
            ...
            "measure_type_description": "Preference duty (fixture)",
            "geographical_area_id": chosen_group,
            "geographical_area_description": chosen_group,
            "components": [{"duty_amount": pref.get("rate_pct") or 0}],
        })
    return {
...
def import_requirements(
    ...
    return {"measures": [], "duty_measures": [], "cert_documents": []}
