...
benchmark sweep needs. The trader journey is single-session, single-user; we
just need:
...
class SessionFacts:
    ...
    No locking, no persistence - one user's session lives in one Python dict.
    """
...
async def run_qa_session(
    ...
    if strategy == "mcp_skill":
        from journey import mcp_skill
        return await mcp_skill.run_mcp_skill_session(
            query=query, max_rounds=max_rounds, oracle_text=oracle_text,
            config=config, human_answers=human_answers,
        )
